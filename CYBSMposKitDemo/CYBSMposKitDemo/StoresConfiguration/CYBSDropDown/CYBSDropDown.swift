...
public typealias CellConfigurationClosure = (Index, String, CYBSDropDownCell) -> Void
...
public final class CYBSDropDown: UIView {
	...
	public static weak var VisibleDropDown: CYBSDropDown?

	//MARK: UI
	fileprivate let dismissableView = UIView()
	fileprivate let tableViewContainer = UIView()
	fileprivate let tableView = UITableView()
	fileprivate var templateCell: CYBSDropDownCell!
	...
	*/
	...
	//MARK: Appearance
	@objc public dynamic var cellHeight = CYBSDropDownConstants.UI.RowHeight {
		willSet { tableView.rowHeight = newValue }
		didSet { reloadAllComponents() }
	}

	@objc fileprivate dynamic var tableViewBackgroundColor = CYBSDropDownConstants.UI.BackgroundColor {
		willSet {
	...
	*/
	@objc public dynamic var selectionBackgroundColor = CYBSDropDownConstants.UI.SelectionBackgroundColor
	...
	*/
	@objc public dynamic var separatorColor = CYBSDropDownConstants.UI.SeparatorColor {
	...
	*/
	@objc public dynamic var cornerRadius = CYBSDropDownConstants.UI.CornerRadius {
		willSet {
	...
	*/
	@objc public dynamic var shadowColor = CYBSDropDownConstants.UI.Shadow.Color {
	...
	Changing the shadow color automatically reloads the drop down.
	*/
	@objc public dynamic var shadowOffset = CYBSDropDownConstants.UI.Shadow.Offset {
	...
	*/
	@objc public dynamic var shadowOpacity = CYBSDropDownConstants.UI.Shadow.Opacity {
	...
	*/
	@objc public dynamic var shadowRadius = CYBSDropDownConstants.UI.Shadow.Radius {
	...
	*/
	@objc public dynamic var animationduration = CYBSDropDownConstants.Animation.Duration

	/**
	The option of the show animation. Global change.
	*/
	public static var animationEntranceOptions = CYBSDropDownConstants.Animation.EntranceOptions
	
	/**
	The option of the hide animation. Global change.
	*/
	public static var animationExitOptions = CYBSDropDownConstants.Animation.ExitOptions
	
	/**
	The option of the show animation. Only change the caller. To change all drop down's use the static var.
	*/
	public var animationEntranceOptions: UIView.AnimationOptions = CYBSDropDown.animationEntranceOptions
	
	/**
	The option of the hide animation. Only change the caller. To change all drop down's use the static var.
	*/
	public var animationExitOptions: UIView.AnimationOptions = CYBSDropDown.animationExitOptions

	/**
	The downScale transformation of the tableview when the DropDown is appearing
	*/
	public var downScaleTransform = CYBSDropDownConstants.Animation.DownScaleTransform {
	...
	*/
	@objc public dynamic var textColor = CYBSDropDownConstants.UI.TextColor {
		didSet { reloadAllComponents() }
	}
    ...
    /**
     ...
     Changing the text color automatically reloads the drop down.
     */
    @objc public dynamic var selectedTextColor = CYBSDropDownConstants.UI.SelectedTextColor {
        didSet { reloadAllComponents() }
    }
	...
	*/
	@objc public dynamic var textFont = CYBSDropDownConstants.UI.TextFont {
		didSet { reloadAllComponents() }
	}
     ...
     */
	public var cellNib = UINib(nibName: "CYBSDropDownCell", bundle: Bundle(for: CYBSDropDownCell.self)) {
		didSet {
			tableView.register(cellNib, forCellReuseIdentifier: CYBSDropDownConstants.ReusableIdentifier.DropDownCell)
			templateCell = nil
			reloadAllComponents()
		}
	}
...
private extension CYBSDropDown {
...
extension CYBSDropDown {
	...
	fileprivate func computeLayoutBottomDisplay(window: UIWindow) -> ComputeLayoutTuple {
		...
		let windowMaxY = window.bounds.maxY - CYBSDropDownConstants.UI.HeightPadding - offsetFromWindowBottom
		
		let keyboardListener = CYBSKeyboardListener.sharedInstance
		let keyboardMinY = keyboardListener.keyboardFrame.minY - CYBSDropDownConstants.UI.HeightPadding
	...
	fileprivate func computeLayoutForTopDisplay(window: UIWindow) -> ComputeLayoutTuple {
		...
		let windowY = window.bounds.minY + CYBSDropDownConstants.UI.HeightPadding
	...
	fileprivate func fittingWidth() -> CGFloat {
		if templateCell == nil {
			templateCell = (cellNib.instantiate(withOwner: nil, options: nil)[0] as! CYBSDropDownCell)
		}
...
extension CYBSDropDown {
	...
	*/
    ...
    public func show(onTopOf window: UIWindow? = nil, beforeTransform transform: CGAffineTransform? = nil, anchorPoint: CGPoint? = nil) -> (canBeDisplayed: Bool, offscreenHeight: CGFloat?) {
		if self == CYBSDropDown.VisibleDropDown && CYBSDropDown.VisibleDropDown?.isHidden == false { // added condition - DropDown.VisibleDropDown?.isHidden == false -> to resolve forever hiding dropdown issue when continuous taping on button - Kartik Patel - 2016-12-29
			return (true, 0)
		}

		if let visibleDropDown = CYBSDropDown.VisibleDropDown {
			visibleDropDown.cancel()
		}

		willShowAction?()

		CYBSDropDown.VisibleDropDown = self
	...
	/// Hides the drop down.
	public func hide() {
		if self == CYBSDropDown.VisibleDropDown {
			/*
			...
			*/
			CYBSDropDown.VisibleDropDown = nil
		}
...
extension CYBSDropDown: UITableViewDataSource, UITableViewDelegate {
	...
	public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		let cell = tableView.dequeueReusableCell(withIdentifier: CYBSDropDownConstants.ReusableIdentifier.DropDownCell, for: indexPath) as! CYBSDropDownCell
	...
	fileprivate func configureCell(_ cell: CYBSDropDownCell, at index: Int) {
